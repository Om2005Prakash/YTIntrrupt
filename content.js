if (!window.timerinit) {
  ...
  box.className = "ytIntrrupt-box";
  box.innerHTML = `
    <div class="eagle-container">

      <svg class="eagle-icon hidden" width="800px" height="800px" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" fill="#000000">
    ...
    <span class="timer-label">0 min</span>
  `;
  ...
  // Style
  if (!document.getElementById("ytIntrrupt-styles")) {
    const style = document.createElement("style");
    style.id = "ytIntrrupt-styles";
    style.textContent = `
      .ytIntrrupt-box {
        position: fixed;
        top: 10px;
      ...
      .ytIntrrupt-box.work-mode .eagle-icon {
        opacity: 1;
  ...
  window.timer = box.querySelector(".timer-label");
}
window.timerinit = true;

function removeExistingTimer() {
  const existing = document.querySelectorAll(".ytIntrrupt-box");
